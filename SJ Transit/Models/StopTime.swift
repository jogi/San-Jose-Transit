...
class StopTime: NSObject, MKAnnotation {
    ...
    class func stopTimes(stopId: String!, afterTime: NSDate!, tripIds: Array<String>!) -> (Array<StopTime>) {
        ...
        let query = stopTimes.select(stopTimes[colArrivalTime].min, trips[colRouteId], routes[colRouteShortName], trips[colDirectionId], trips[colTripHeadSign]).join(trips, on: stopTimes[colTripId] == trips[colTripId]).join(routes, on: trips[colRouteId] == routes[colRouteId]).filter(stopTimes[colStopId] == stopId && tripIds.contains(stopTimes[colTripId]) && stopTimes[colArrivalTime] >= DateFormatter.SQLTimeFormatter.stringFromDate(afterTime) && stopTimes[colIsLastStop] == false).group(trips[colRouteId]).order(trips[colRouteId], stopTimes[colArrivalTime])
        
        do {
            for stopTime in try db.prepare(query) {
                let aStopTime = StopTime()
                aStopTime.arrivalTime = DateFormatter.SQLTimeFormatter.dateFromString(stopTime[stopTimes[colArrivalTime].min]!.sanitizedTimeString)
    ...
    class func stopTimes(stopId: String!, routeId: String!, tripIds: Array<String>!, afterTime: NSDate!) -> (Array<StopTime>) {
        ...
        let query = stopTimes.select(stopTimes[colArrivalTime], trips[colTripHeadSign], trips[colTripId], trips[colShapeId], trips[colDirectionId]).join(trips, on: stopTimes[colTripId] == trips[colTripId]).filter(stopTimes[colStopId] == stopId && trips[colRouteId] == routeId && tripIds.contains(stopTimes[colTripId]) && stopTimes[colArrivalTime] >= DateFormatter.SQLTimeFormatter.stringFromDate(afterTime) && stopTimes[colIsLastStop] == false).order(stopTimes[colArrivalTime])
        
        do  {
            for stopTime in try db.prepare(query) {
                let aStopTime = StopTime()
                aStopTime.arrivalTime = DateFormatter.SQLTimeFormatter.dateFromString(stopTime[stopTimes[colArrivalTime]].sanitizedTimeString)
    ...
    class func trip(routeId: String!, directionId: Direction!, afterTime: NSDate!) -> (String?) {
        ...
        do {
            for row in try db.prepare(stopTimes.select(colTripId).filter(tripIds.contains(colTripId) && colStopSequence == 1 && colArrivalTime >= DateFormatter.SQLTimeFormatter.stringFromDate(afterTime)).order(colArrivalTime).limit(1)) {
